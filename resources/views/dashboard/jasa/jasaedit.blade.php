...
<div class="col-lg-8">
    <form method="post" action="/product/{{$pemesananjasa->id}}" class="mb-5" enctype="multipart/form-data">
        @method('put')
        @csrf
        ...
        <div class="mb-3">
            <label for="image_develop" class="form-label @error('image_develop') is-invalid @enderror">Develop Image</label>
            <input type="hidden" name="oldImage" value="{{ $pemesananjasa->image_develop }}">
            @if($pemesananjasa->image_develop)
            <img src="{{ asset('storage/' . $pemesananjasa->image_develop) }}" class="img-preview img-fluid mb-3 col-sm-5 d-block">
            <input class="form-control" type="file" id="image_develop" name="image_develop" onchange="previewImage()">
            @else
...
<script>
    ...
    function previewImage() {
        const image = document.querySelector('#image_develop');
